...
public:
    ...
    curried(Function function, CapturedArgs... args)
        : m_function(function)
        , m_captured(capture_by_body(std::forward<CapturedArgs>(args)...))
    {
    }
    ...
    {
        auto new_args = capture_by_body(std::forward<NewArgs>(args)...);
...
private:
    ...
    static auto capture_by_body(Args... args) {
...
{
    auto mycurried_print = make_curried(print);
    mycurried_print(1)(2)(3);
